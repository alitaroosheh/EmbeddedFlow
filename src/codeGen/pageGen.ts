...
import { bindingsInitCalls } from "./bindingsGen";
...
export function generateRootSource(project: EmbfProject): string {
    ...
    const stylesInit = (project.styles?.length ?? 0) > 0
        ...
        : [];
    const bindingsCalls = bindingsInitCalls(project);
    const bindingsInit = bindingsCalls.length > 0
        ? [`    /* Initialise data bindings (ui_bindings.c) */`, ...bindingsCalls, ``]
        : [];
